...
func _on_response_gui_input(event: InputEvent, item: Control, response) -> void:
	...
	# not sure why this randomly stopped working
	elif event.is_action_pressed(&"ui_down"):
		for button: Control in get_menu_items():
			if button.has_focus():
				button.get_node(button.focus_next).grab_focus()
				break
	elif event.is_action_pressed(&"ui_up"):
		for button: Control in get_menu_items():
			if button.has_focus():
				button.get_node(button.focus_previous).grab_focus()
				break
#endregion
